...
export class AddItemsComponent {
  ...
  openDialog(): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        const variantDetails = result.variant_details;
        result.variants = [];
        result.quantity = [];
        result.price = [];
        result.quantity_type = [];
        variantDetails.forEach((val) => {
          result.variants.push(val.variant);
          result.quantity.push(parseInt(val.quantity, 10));
          result.price.push(parseInt(val.price, 10));
