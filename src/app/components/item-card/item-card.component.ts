...
export class ItemCardComponent {
  ...
  // Open the confirmation tab for deleting a product or its variant
  ...
  openSelectVarietyDialog() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        const deleteDetail = result.split(" ");
        let deleteURL = "";
        ...
        this.productService
          ...
          .subscribe(
            (response) => {
              ...
              this.snackBar.open("Product Detail Updated", "", {
                duration: 5000,
              });
            },
            (error) => {
